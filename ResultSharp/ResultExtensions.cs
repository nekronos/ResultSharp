...
public static class ResultExtensions
{
    ...
    public static TRet Match<T1, T2, E, TRet>(
        (Result<T1, E> a, Result<T2, E> b) tuple,
        Func<T1, T2, TRet> ok, Func<IEnumerable<E>, TRet> err)
    {
        var (a, b) = tuple;
        return (a.IsOk, b.IsOk) switch
        {
            (true, true) => ok(a.Value, b.Value),
            (false, false) => err(new[] { a.Error, b.Error }),
            (false, _) => err(new[] { a.Error }),
            (_, false) => err(new[] { b.Error })
        };
    }
